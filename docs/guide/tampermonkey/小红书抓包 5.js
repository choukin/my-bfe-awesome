...
*/
  ...
  function monitorFeed() {
    ...
    try {
      ...
      exploresData.push({
        ...
        创建时间: new Date(detail.time).toLocaleString()
      })
      setExploresDetail(JSON.stringify(exploresData))
    } catch (e) {
      setStart('')
    ...
    $(function () {
      ...
      if (!hasMore) {
        ...
        if (window.location.href.indexOf('www.xiaohongshu.com/user/profile') > -1) {
          datas = datas.filter(function (item, index) {
            return index < 100
          })
  ...
  function saveExcel() {
    ...
    const wb = window.XLSX.utils.table_to_book(table, { sheet: nickname.replace('History', '英文历史') + userid + '文章统计', raw: true });
  ...
  function exportDetailSheet() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, nickname.replace('History', '英文历史'));
    XLSX.writeFile(wb, nickname.replace('History', '英文历史') + ".xlsx");
  }
  ...
  function init() {
    const div = $('<div id="insepctAPP" class="insepctAPP"><button type="button" class="layui-btn" lay-on="prompt">设置数据</button></div>').click(() => {
      layer.prompt({
        title: '输入要看的网络地址多个用,分割; 每个账户可以获取最近 100 篇文章',
        formType: 2,
        btn: ['开始解析', '取消'],
